...
public class IlluminationTesterTest {
	...
	public void testTestIfRoomIsIlluminatedIteratorOfLampHashSetOfIntegerIRuntimeIlluminationTester() {
		...
		lamp2.turnOn();
		lamps.add(lamp2);
		boolean lampsHaveAllTags =
				illuminationTester.testIfRoomIsIlluminated(lamps.iterator(), allTags, null);
		
		//Assert
		assertFalse("No tags are included. The test should have returned false!",
				lampIsNotTagged);
		assertTrue("All tags are associated with lamps. The test should have returned true!",
				lampsHaveAllTags);
	}
	
	// The other method with parameters HashSet<Integer>, HashSet<Integer> is trivial and not tested
