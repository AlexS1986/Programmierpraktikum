...
public class CandidateSearcher4 implements ICandidateSearcher {
	...
	public List<Lamp> searchCandidates(IRoom room, IRuntimeCandidateSearcher runtimeCandidateSearcher)
		...
		while (combinationsOccured) {
			...
			for (int i = 0; i < inPartialRectangles.size(); i++) {
				...
				for (int j = i + 1; j < inPartialRectangles.size(); j++) {
					...
					if (!recI.equals(rIJ)) { // sth happended
						Rectangle recIr = new Rectangle(recI.getP1(), recI.getP3());
						Rectangle recIJr = new Rectangle(rIJ.getP1(), rIJ.getP3());

						/*if (recIr.equals(recIJr)) { // tags do not agree
							for (Integer tag : recI.getCopyOfTags()) {
								rIJ.addTag(tag);
							}
						...
						if (outPartialRectanglesIn.contains(recI)) { // only add if empty
							outPartialRectanglesIn.add(recI);
						}
					}
				}
		...
		outPartialRectangles = inPartialRectangles;
