		...
		try {
			for (String paramString : args) {
				...
				if(st.countTokens()==2) {
					...
					String paramterValue = st.nextToken().trim();
					switch(parameterKey) {	
					case "r":
						parameterSet.setRunParameter(paramterValue);
						break;
					case "if":
						parameterSet.setInputFile(paramterValue);
						break;
					case "l":
						parameterSet.setTimeLimit(Integer.parseInt(paramterValue));
						break;
					default:
			...
			if (parameterSet.isValidParameterSet() ) {
				...
				case "s":
					...
					userSolveAAS.solve(room, parameterSet.getTimeLimit());
					break;
				case "sd":
					userReadWriteAAS = new UserReadInputWriteOutputAAS(parameterSet.getInputFile());
					room = userReadWriteAAS.readInput();
