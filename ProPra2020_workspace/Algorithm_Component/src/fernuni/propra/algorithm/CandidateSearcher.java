...
public class CandidateSearcher implements ICandidateSearcher {
	...
	public List<Lamp> searchCandidates(IRoom room, IRuntimeCandidateSearcher runtimeCandidateSearcher)
		...
		try {
			...
			runtimeCandidateSearcher.startTimeOriginalPartialRectanglesFind(); // store runtime for construction of
																				// original partial rectangles
			ArrayList<RectangleWithTag> originalRectangles = AbstractAlgorithmFactory.getAlgorithmFactory()
			...
			// reduce rectangles: result is non overlapping set of rectangles. Each
			// rectangle contains all tags of
	 ...
	 */
	ArrayList<RectangleWithTag> reduceRectangles(ArrayList<RectangleWithTag> originalRectanglesTagged)
			throws InterruptedException {
		...
		do { // as long as set of rectangles can still be reduced
			...
			ArrayList<RectangleWithTag> intersectedRectangleWithTags = new ArrayList<RectangleWithTag>(); // set of
																											// rectangles
																											// with tag
																											// that can
																											// be
			...
			for (int i = 0; i < reducedRectanglesLastIteration.size(); i++) {
				...
				for (int j = i + 1; j < reducedRectanglesLastIteration.size(); j++) {
					...
					if (overlappingRectangle != null) { // intersection detected
						...
						// and add that to the tags of the overlap
						for (int k = j + 1; k < reducedRectanglesLastIteration.size(); k++) {
							...
							if (overlappingRectangle.getCenter().isInsideRectangle(rectangleWithTagK.getP1(),
									rectangleWithTagK.getP3())) {
								tagsOfOverlap.addAll(rectangleWithTagK.getCopyOfTags());
							}
						}

						// add to all new rectangles
						RectangleWithTag overlappingRectangleWithTag = new RectangleWithTag(overlappingRectangle,
								tagsOfOverlap);
						intersectedRectangleWithTags.add(overlappingRectangleWithTag);
					}
				}
				// still need to keep rectangleWithTagI if no intersection with other rectangles
				// is found
				if (!intersectFoundI) {
			...
			// determine those rectangles whose tags are not contained in another
			// rectangle's tags
			reducedRectanglesCurrentIteration = new ArrayList<RectangleWithTag>();
			boolean[] isMinRectangle = new boolean[intersectedRectangleWithTags.size()]; // isMinRectangle[i] = true ->
																							// tags of rec_i are not
			...
			for (int i = 0; i < intersectedRectangleWithTags.size(); i++) {
				...
				for (int j = i + 1; j < intersectedRectangleWithTags.size(); j++) {
					...
					boolean iSubsetOfJ = rectangleWithTagJ.getCopyOfTags()
							.containsAll(rectangleWithTagI.getCopyOfTags());
					boolean jSubsetOfI = rectangleWithTagI.getCopyOfTags()
							.containsAll(rectangleWithTagJ.getCopyOfTags());
			...
			// keep only those rectangles whose tags are not contained in another
			// rectangle's tags for next iteration
			for (int i = 0; i < isMinRectangle.length; i++) {
				if (isMinRectangle[i]) {
