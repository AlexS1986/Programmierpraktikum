...
/**
 * A provider of an algorithm that can find the original partial rectangles for an {@link IRoom} instance.
 * <p>
...
public interface IOriginalPartialRectanglesFinder {
	/**
	 * The original partial rectangles of an {@link IRoom} are tagged with {@link Integer}s that 
	 * denote "parts" (e.g. walls) of the room that are illuminated if the associated partial rectangle is illuminated.
	 * If all tags are illuminated then the room is illuminated. 
	 * This method returns a set that contains all tags of all original partial rectangles of the {@link IRoom}.
	 * <p>
	 ...
	 * to the instance of {@link IRuntimeOriginalPartialRectanglesFinder}. All rectangles are tagged with Integers that denote
	 * the parts of the room (e.g. walls) that are illuminated if the rectangle is illuminated. If all tags are illuminated then it 
	 * must follow that the room is illuminated. Identical rectangles are only stored once.
