...
public class PositionOptimizer implements IPositionOptimizer {
	...
	/**
	 * Implements the backtracking algorithm as indicated in the commentary on the
	 * {@link PositionOptimizer} class.
	 * 
	...
	private void searchSolution(ArrayList<Lamp> lamps, int idx, HashSet<Integer> allTags, int numberLampsOn,
		...
		if (illuminationTester.testIfRoomIsIlluminated(lamps.iterator(), allTags,
				runTimeInformation)) { /*
										 * valid solution found, else case does not need to be investigated since it all
										 * lamps with an idx larger than the supplied idx are already turned on
										 */
