...
public abstract class WallContainerAbstract implements Iterable<Wall> {
	 ...
	 * by the implementing subclass). The search can be further specified by defining a range of doubles,
	 ...
	 * @param limit : limit to be used to find the next wall according to the ordering.
	...
	public Wall getNearestWall(double low, double high, double limit) throws WallContainerException{
		if (low > high) throw new WallContainerException(); // not a valid range
		Iterator<Wall> iterator = walls.iterator(); // walls are ordered
		Wall nextWall;
		while(iterator.hasNext()) {
			nextWall = iterator.next();
			if (isValidWall(nextWall, limit, low, high)) { // wall fits the  range and is also the next wall according to the ordering
				return nextWall;
			}
		}
		return null;
	 ...
	 * @param low : lower end of the range that is used to further specify the search for the next {@link Wall}
	 * @param high : higher end of the range that is used to further specify the search for the next {@link Wall}
	 * @return a boolean that shows whether the {@link Wall} is a valid {@link Wall} that matches the semantics of
	 * {@link getNearestWall}
	 */
	...
	/**
	 * 
	 * @return A {@link Comparator}<{@link Wall}> that specifies the total ordering that is used in this container. 
	 */
