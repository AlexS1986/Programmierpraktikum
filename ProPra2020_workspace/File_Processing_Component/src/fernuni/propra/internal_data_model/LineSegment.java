 ...
 * <p>
 * The line segment is defined by a starting {@link Point} P1, and an end
 * {@link Point} P2.
 * <p>
 * The {@link LineSegment} provides functionality, such as
 * <p>
 * 1.) Tests that check whether the {@link LineSegment} is horizontal or
 * vertical
 * <p>
 * 2.) A test that checks whether the projection of that {@link LineSegment} on
 * the x-/y-axis overlaps a specified x- or y- range
 * <p>
 * 3.) A check whether this {@link LineSegment} is perpendicular to another
 * {@link LineSegment}.
 * <p>
 * 4.) A check that computes whether this {@link LineSegement} penetrates any
 * other {@link LineSegment}s of a provided set.
 * <p>
...
public class LineSegment {
	 ...
	 * @param xLow
	 * @param xHigh
	 * @return A boolean that represents whether this {@link LineSegment} overlaps a
	 *         range defined by xLow <= x <=xHigh when this {@link LineSegment} is
	 *         projected on the x-axis.
	 */
	public boolean overlapsXrange(double xLow, double xHigh) {
		if (xLow > xHigh)
			throw new IllegalArgumentException("xLow > xHigh! Insert a valid range!");
		// pre xLow < xHigh
	 ...
	 * @param yHigh
	 * @return A boolean that represents whether this {@link LineSegment} overlaps a
	 *         range defined by yLow <= y <=yHigh when this {@link LineSegment} is
	 *         projected on the y-axis.
	 */
	public boolean overlapsYrange(double yLow, double yHigh) {
		if (yLow > yHigh)
			throw new IllegalArgumentException("yLow > yHigh! Insert a valid range!");
		// pre yLow < yHigh
	...
	/**
	 * 
	 * @param other : {@link LineSegment} that is to be be tested whether its
	 *              perpendicular to this {@link LineSegment}
	 * @return
	 */
	...
	/**
	 * A test that checks whether this {@link LineSegment}s penetrates any of the
	 * {@link LineSegment}s in a provided set of {@link LineSegment}s
	 * 
	 * @param lineSegments : the set of {@link LineSegment}s to be checked
	 * @return A boolean that states whether this {@link LineSegment}s penetrates
	...
	public boolean penetratesLineSegments(List<LineSegment> lineSegments) {
		...
		for (int j = 0; j < lineSegments.size(); j++) {
			...
			try {
				...
				if (!intersectionPoint.equals(this.getP1()) && !intersectionPoint.equals(this.getP2())) {
					penetrates = true;
					break;
				}
	...
	/**
	 * Computes the intersection of this {@link LineSegment} with another
	 * {@link LineSegment}. The intersection is not computed for line segments but
	 * for the complete infinitely long lines defined by the {@link LineSegment}s
	 * 
	 * @param other : another {@link LineSegment}
	 * @return A {@link Point} that represents the intersection of this
	 *         {@link LineSegment} with "other".
	 * @throws LineSegmentException : If {@link LineSegments} are not perpendicular.
	 */
	public Point intersectionWithLinesegment(LineSegment other) throws LineSegmentException {
		if (perpendicular(other)) {
			...
			throw new LineSegmentException(
					"Lines are not perpendicular cannot" + " return (unique) intersection point!");
		}
