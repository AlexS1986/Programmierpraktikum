 ...
 * <p>
 * In this component, among other things the input parameters are parsed and
 * handled.</br>
 * <p>
 * According to the choice of the parameters, the program flow is delegated to
...
public class Main {
	...
	private static final String generalHelpMessage = "Could not run the program. Please call the program correctly."
			+ ls + ls + "The correct syntax is:" + ls + ls + "Java -jar ProPra.jar r=runParameter "
			+ "if=\"pathToFile\" l=timeLimit" + ls + ls + "The runParameter specified by r =runParameter is mandatory "
			+ "and must be one of s,sd,v,vd or d ." + ls + "The input file parameter is also mandatory. pathToFile "
			+ "specifies the full path to a valid input file. " + "The \" before and after pathToFile are mandatory"
			+ ls + "The time limit parameter is optional. " + "For runParameter = s or runParameter = sd. "
			+ "This parameter specifies how long the solution " + "algorithm searches for an optimal lamp layout. "
			+ "Time limit must be a positive integer number.";

	/**
	 * General access point to the functionality of this program.
	 * 
	 * @param args Three input parameters have to be provided as command line
	 *             parameters in the form r=value_r if=pathToFile l=timeLimit.
	 *             <p>
	 *             The control flow parameter r is treated as follows:
	 *             <ul>
	 * 
	 *             <li>r="s" (solve): for the problem instance described by the
	 *             given xml-file a solution is computed. The computed lamp
	 *             positions are saved to the given xml-file. Already existing lamp
	 *             positions will be replaced in the xml-file.</li>
	 * 
	 *             <li>r="sd" (solve & display): like r="s", except for the fact
	 *             that the room, including lamps, is also displayed graphically.
	 *             </li>
	 * 
	 *             <li>r="v" (validate): this option checks whether the room defined
	 *             in the given xml-file is illuminated by its lamps. The result as
	 *             well as the number of lamps is printed to the command line
	 *             display. .</li>
	 * 
	 *             <li>r="vd" (validate & display): like r="v", except for the fact
	 *             that after validation the room is also displayed graphically.
	 *             </li>
	 * 
	 *             <li>r="d" (display): The room given by the specified xml-file,
	 *             including lamps is displayed graphically.</li>
	 *             </ul>
	 *             The input parameter "if" is a String that that defines the
	 *             absolute path to a valid xml-file that describes an room
	 *             according to the format specified in [1]. If the room is not
	 *             valid, an error message is printed to the command line display.
	 *             <p>
	 *             The parameter time limit parameter "l" is used to s pecify a time
	 *             limit in seconds after which the computation will be aborted for
	 *             the cases r="s" and r = "sd". The best available setting of lamps
	 *             is displayed if a solution has been found, if not then no changes
	 *             will be made to the xml-file and also nothing will be displayed,
	 *             except for a warning on the console.
	 *             <p>
	 *             [1] Aufgabenstellung Programmierpraktikum Fernuni Hagen
	 *             Sommersemester 2020, Listing 1
	 */
		...
		try {
			...
			if (parameterSet.isValidParameterSet()) {
				...
				case "s":
					...
					// to command line
					printMessageToConsole("Number of lamps necessary:" + numberOfLampsInSolution);
			...
			printMessageToConsole("The file could not be read correctly. Please provide a valid file.");
			System.out.println("Shutting down.");
			System.exit(0);
