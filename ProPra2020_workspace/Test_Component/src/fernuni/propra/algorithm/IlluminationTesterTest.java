...
public class IlluminationTesterTest {
	...
	public void testTestIfRoomIsIlluminatedIRoomIRuntimeIlluminationTester() {
		// Arrange
		IIlluminationTester illuminationTester = AbstractAlgorithmFactory.getAlgorithmFactory()
		...
		try {
			// Room 1
			roomSquareWOLamp = illuminationTester.testIfRoomIsIlluminated(roomSquare, new RuntimeInformation());
			Lamp lamp = new Lamp(0.5, 0.5);
			lamp.turnOff();
			roomSquare.addLamp(lamp);
			roomSquare1LampOff = illuminationTester.testIfRoomIsIlluminated(roomSquare, new RuntimeInformation());
			lamp.turnOn();
			roomSquare1LampOn = illuminationTester.testIfRoomIsIlluminated(roomSquare, new RuntimeInformation());
			lamp.turnOff();
			...
			roomSquare1LampOnOutside = illuminationTester.testIfRoomIsIlluminated(roomSquare, new RuntimeInformation());

			// lamp on corner
			Lamp lamp3 = new Lamp(1.0, 1.0);
			lamp3.turnOn();
			roomSquare.addLamp(lamp3);
			roomSquareLampOnCorner = illuminationTester.testIfRoomIsIlluminated(roomSquare, new RuntimeInformation());
		...
		// Assert
		assertFalse("roomSquare cannot be illuminated without a lamp.", roomSquareWOLamp);
		assertFalse("roomSquare cannot be illuminated with just 1 lamp that is off", roomSquare1LampOff);
		assertTrue("roomSquare must be illuminated with 1 lamp in center!", roomSquare1LampOn);
		assertFalse("roomSquare cannot be illuminated if lamp is outside the room", roomSquare1LampOnOutside);
		assertTrue("roomSquare must be illuminated with 1 lamp on a corner", roomSquareLampOnCorner);

		assertFalse("roomHufeisen cannot be illuminated without a lamp", roomHufeisenWOLamp);
		assertFalse("roomHufeisen cannot be illuminated with 1 lamp", roomHufeisen1Lamp);
		assertTrue("roomHufeisen must be illuminated in this specific setup!", roomHufeisen2Lamp);

	}

	/**
	 * Checks whether {@link IlluminationTester} works correctly, if a list of
	 * tagged {@link Lamp}s is given as parameter.
	 */
	@Test
	public void testTestIfRoomIsIlluminatedIteratorOfLampHashSetOfIntegerIRuntimeIlluminationTester() {
		...
		// Act
		boolean lampIsNotTagged = illuminationTester.testIfRoomIsIlluminated(lamps.iterator(), allTags, null);
		Lamp lamp2 = new Lamp(0, 0, 1);
		lamp2.addTag(2);
		lamp2.addTag(3);
		lamp2.turnOn();
		lamps.add(lamp2);
		boolean lampsHaveAllTags = illuminationTester.testIfRoomIsIlluminated(lamps.iterator(), allTags, null);

		// Assert
		assertFalse("No tags are included. The test should have returned false!", lampIsNotTagged);
		assertTrue("All tags are associated with lamps. The test should have returned true!", lampsHaveAllTags);
	}

	// The other method with parameters HashSet<Integer>, HashSet<Integer> is
	// trivial and not tested
