...
public class OriginalPartialRectanglesFinderTest {
	...
	public void setUp() throws Exception {
		...
		cornersSquare.add(p1);
		cornersSquare.add(p2);
		cornersSquare.add(p3);
		...
		roomL = new Room("test", null, cornersL);
	 ...
	 * {@link OriginalPartialRectanglesFinder} can compute the set of all tags that
	 * make up the room.
	 */
	@Test
	public void testGetAllTags() {
		...
		// Assert
		assertTrue("Did not return the correct tags.", rectanglesFinder.getAllTags().containsAll(refSet));
	}

	/**
	 * Checks whether walls are sorted correctly into containers, according to their
	 * orientation.
	 */
	@Test
	public void testSortWallsToContainers() {
		...
		assertTrue("East walls not sorted correctly", eastBool);
		assertTrue("North walls not sorted correctly", northBool);
		assertTrue("West walls not sorted correctly", westBool);
		assertTrue("South walls not sorted correctly", southBool);
	}

	/**
	 * Checks whether original partial rectangles are constructed correctly for
	 * simple rooms.
	 */
	@Test
	public void testConstructOriginalPartialRectangles() {
		// Arrange
		OriginalPartialRectanglesFinder rectanglesFinderRoomSquare = new OriginalPartialRectanglesFinder();
		OriginalPartialRectanglesFinder rectanglesFinderRoomL = new OriginalPartialRectanglesFinder();

		// Act
		try {
			rectanglesFinderRoomSquare.sortWallsToContainers(roomSquare);
			rectanglesFinderRoomSquare.constructOriginalPartialRectangles();
		} catch (WallContainerException e) {
			fail(e.getMessage());
		} catch (OriginalPartialRectanglesFinderException e) {
			fail(e.getMessage());
		...
		// 2nd room
		try {
			rectanglesFinderRoomL.sortWallsToContainers(roomL);
			rectanglesFinderRoomL.constructOriginalPartialRectangles();
		} catch (WallContainerException e) {
			fail(e.getMessage());
		} catch (OriginalPartialRectanglesFinderException e) {
			fail(e.getMessage());
		...
		// Assert
		// 1st room
		Iterator<RectangleWithTag> roomSquareIterator = rectanglesFinderRoomSquare.iteratorOriginalRectangles();
		RectangleWithTag rec1 = roomSquareIterator.next();
		RectangleWithTag ref = new RectangleWithTag(p1, p3, 1);
		ref.addTag(2);
		ref.addTag(3);
		ref.addTag(4);

		boolean only1Rectangle = !roomSquareIterator.hasNext();
		boolean correctRectangle = rec1.equals(ref);

		assertTrue("More than one rectangle has been returned for" + "square room!", only1Rectangle);
		assertTrue("The correct rectangle has been returned", correctRectangle);

		// 2nd room
		Iterator<RectangleWithTag> roomLIteator = rectanglesFinderRoomL.iteratorOriginalRectangles();
		RectangleWithTag rec2_1 = roomLIteator.next();
		RectangleWithTag rec2_2 = roomLIteator.next();
		...
		boolean only2Rectangles = !roomLIteator.hasNext();
		boolean correctRectangle1 = rec2_1.equals(ref2) || rec2_1.equals(ref3);
		boolean correctRectangle2 = rec2_2.equals(ref3) || rec2_2.equals(ref2);

		assertTrue("2 rectangles need to be returned", only2Rectangles);
		assertTrue("The returned rectangles are not correct", correctRectangle1);
		assertTrue("The returned rectangles are not correct", correctRectangle2);
	}

	/**
	 * Checks whether the candidates rectangles can be found for a complicated room
	 */
	@Test
	public void testConstructOriginalPartialRectanglesComplicated() {
		// Arrange
		...
		// Assert
		assertTrue("47 original partial rectangles have not" + "been found for Zufallsraum_144! Failed!",
				originalPartialRectangles.size() == 47);
	}
