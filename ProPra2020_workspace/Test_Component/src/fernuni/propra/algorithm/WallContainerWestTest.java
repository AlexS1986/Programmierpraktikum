...
public class WallContainerWestTest {
	...
	/**
	 * tests whether west wall can be added and whether a south wall throws an
	 * exception.
	 */
	@Test
	public void testAdd() {
		...
		// Assert
		assertTrue("Adding a south wall should throw an exception", southWallAddedWException);
		assertTrue("Adding a west wall should not throw an exception", westWallAddedWOException);
	}

	/**
	 * Tests whether the nearest west wall is indeed returned.
	 */
	@Test
	public void testGetNearestWestWall() {
		...
		Wall w11 = null;
		try {
			w11 = wallContainerWest.getNearestWall(-1, 1, 1.0); // right expected
		} catch (WallContainerException e) {
			fail(e.getMessage());
		}
		boolean isRightWall = w11.getP1().equals(w6.getP1()) && w11.getP2().equals(w6.getP2());

		Wall w12 = null;
		try {
			w12 = wallContainerWest.getNearestWall(-1, 1, 1.001); // right expected
		} catch (WallContainerException e) {
			fail(e.getMessage());
		}
		boolean isRightWall2 = w12.getP1().equals(w6.getP1()) && w12.getP2().equals(w6.getP2()); // null expected
		...
		boolean isNull = w13 == null;

		// Assert
		assertTrue(isLeftWall);
		assertTrue(isRightWall);
		assertTrue(isRightWall2);
		assertTrue(isNull);
