...
public class FilePersistenceTest {
	...
	/**
	 * Checks whether the procedure for adding only valid (walls that dont intersect
	 * the current wall set) walls works.
	 */
	@Test
	public void testTestWallAndAddToWalls() {
		...
		try {
			FilePersistence.testAndAddWallToWalls(new LineSegment(p5, p6), walls8);
		} catch (PersistenceException e) {
			intersectionDetected = true;
		}

		// Assert
		assertEquals("A reference wall could not be added.", walls1.get(0), lccw1);
		assertEquals("A reference wall could not be added.", walls2.get(1), lccw2);
		assertEquals("A reference wall could not be added.", walls3.get(2), lccw3);
		assertEquals("A reference wall could not be added.", walls4.get(3), lccw4);
		assertEquals("A reference wall could not be added.", walls5.get(0), l1);
		assertEquals("A reference wall could not be added.", walls6.get(1), l2);
		assertEquals("A reference wall could not be added.", walls7.get(2), l3);
		assertEquals("A reference wall could not be added.", walls8.get(3), l4);
		assertTrue("Intersection of walls should have been" + "detected.", intersectionDetected);

	}

	/**
	 * Checks whether correct input files are read correctly and whether incorrect
	 * input files throw an exception.
	 */
	@Test
	public void testReadInput() {
		...
		for (String xmlFile : xmlPathesOK) {
			...
			try {
				...
				fail("This room should have been read." + e.getMessage());
			}
		}
		...
		for (String xmlFile : xmlPathesNOK) {
			...
			try {
				room = persistence.readInput(xmlFile);
				fail("This xml file is not OK!" + "It should not have been read " + "without exception" + xmlFile);
			} catch (PersistenceException e) {
	...
	public void testWriteOutput() {
		...
		// Assert
		assertTrue("This output write operation should have worked", writeCorrectOutputWorked);
		assertTrue("his output write operation should not have worked", writeInCorrectOutputDidNotWork);
